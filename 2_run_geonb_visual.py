...
class SpatialQueryApp(QWidget):
    def __init__(self):
        ...
        all_cols = pd.concat([buildings_gdf, flood_gdf], axis=0)
        ...
        for col in numeric_cols:
            ...
            def toggle_spin(state, min_box=min_box, max_box=max_box):
                min_box.setEnabled(state == Qt.Checked)
                max_box.setEnabled(state == Qt.Checked)

            checkbox.stateChanged.connect(toggle_spin)
    ...
    def plot_datasets(self):
        self.ax1.clear()
        self.ax2.clear()
        buildings_gdf.plot(ax=self.ax1, color="orange", edgecolor="k")
        flood_gdf.plot(ax=self.ax2, color="lightgrey", edgecolor="k")
        self.ax1.set_title("Buildings Map")
        self.ax2.set_title("Flood Map")
        self.canvas.draw()
    ...
    def run_query(self):
        ...
        where_clauses = [f"ST_Within(buildings.geometry, ST_GeomFromText('{intersection.wkt}'))",
                         f"ST_Within(flood_risk_areas.geometry, ST_GeomFromText('{intersection.wkt}'))",
                         "ST_Contains(flood_risk_areas.geometry, buildings.geometry)"]
        ...
        sql_text = f"""
SELECT * FROM buildings, flood_risk_areas
WHERE
    {' AND\n    '.join(where_clauses)};
"""
