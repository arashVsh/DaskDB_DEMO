...
from DaskDB.table_information import (
    get_dataframe_info,
    set_dataframe_info,
    get_all_table_Names,
)
...
def test():
    ...
    naturalearth_lowres = naturalearth_lowres[
        naturalearth_lowres["continent"] == "Asia"
    ]
...
class Context:
    ...
    def setup_configuration(
        ...
        hdfsMasterIP=None,
        hdfsPort=None,
        daskSchedulerIP=None,
        daskSchedulerPort=None,
    ):
    ...
    def register_table(self, table_name, data_path, delimiter=",", col_names=None):
        if data_path.startswith("hdfs://"):
            if data_path.endswith(".shp"):
                df = gpd.read_file(
                    data_path,
                    storage_options={"host": self.hdfsMasterIP, "port": self.hdfsPort},
                )
